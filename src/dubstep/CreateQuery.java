	...
	CreateQuery() throws IOException, ParseException, SQLException
	{
		...
		if(!onDiskPresent){
			...
			while(createColumnsIterator.hasNext()){
				...
				colName.append(tableName);
				colName.append(".");
				
				colName.append(columnDefinition.getColumnName());
				if(colData.toString().startsWith("CHAR")){
					colData = ("CHAR");
				}
				if(primaryKeys.contains(colName)) primaryKeysNumbers.add(j);
				tableSchema.put(colName.toString(), colData);
				if(indexKeys.contains(colName)){
					secondayIndexNumber.put(j,colName.toString());
				}
				j++;
			}
		...
		//Adding into MasterTableDetails
		masterTableDetails = new HashMap<String, HashMap>();
		masterTableDetails.put(tableName, tableDetails);
		
		FileLoader loadFile = new FileLoader();
		loadFile.evaluateTable(tableName);
		//commenting as part of project
//		if(secondayIndex!=null)
//			secondayIndex.clear();
		if(onDiskPresent && tableName.equals("LINEITEM")){
